...
Groups AS (
		SELECT
			...
			FOC2_YQT + KEEP_CQT + KEEP_EDQT + KEEP_FQT + KEEP_YQT)-ISNULL(bt.ORDERED_SEATS,0) AS RESERVABLE_SEATS,
			ISNULL(bt.ORDERED_SEATS,0) AS SOLD_SEATS,
			g.PAX_COUNT,
			TRGRUP.BUD_DTM,
			TRGRUP.ORDER_DL,
			TRGRUP.LEAV_DT,
		...
		WHERE CONVERT(DATE,TRGRUP.LEAV_DT) BETWEEN '{{var("train_start")}}' AND '{{var("train_end")}}'
